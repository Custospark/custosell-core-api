<?php

use Illuminate\Support\Facades\Route;

Route::prefix('v1')->group(function () {
    Route::get('health', function () {
        return response()->json([
            'status' => 'ok',
            'version' => config('app.version'),
            'timestamp' => now()->toIso8601String(),
        ]);
    });
    require __DIR__ . '/api/v1/plans.php';
    require __DIR__ . '/api/v1/users.php';
    require __DIR__ . '/api/v1/businesses.php';
    require __DIR__ . '/api/v1/business-social-links.php';
    require __DIR__ . '/api/v1/locations.php';
    require __DIR__ . '/api/v1/roles.php';
    require __DIR__ . '/api/v1/categories.php';
    require __DIR__ . '/api/v1/products.php';
    require __DIR__ . '/api/v1/inventory.php';
    require __DIR__ . '/api/v1/customers.php';
    require __DIR__ . '/api/v1/shifts.php';
    require __DIR__ . '/api/v1/sales.php';
    require __DIR__ . '/api/v1/orders.php';
    require __DIR__ . '/api/v1/sale_items.php';
    require __DIR__ . '/api/v1/stock_movements.php';
    require __DIR__ . '/api/v1/marketplace.php';
    require __DIR__ . '/api/v1/purchase_orders.php';
    require __DIR__ . '/api/v1/subscriptions.php';
    require __DIR__ . '/api/v1/expense_categories.php';
    require __DIR__ . '/api/v1/expenses.php';
    require __DIR__ . '/api/v1/personal_budgets.php';
    require __DIR__ . '/api/v1/income_sources.php';
    require __DIR__ . '/api/v1/sync.php';
    require __DIR__ . '/api/v1/dashboard.php';
    require __DIR__ . '/api/v1/reports.php';
    require __DIR__ . '/api/v1/platform.php';
    require __DIR__ . '/api/v1/notifications.php';
    require __DIR__ . '/api/v1/webpush.php';
    require __DIR__ . '/api/v1/quick_notes.php';
    require __DIR__ . '/api/v1/linked_accounts.php';
    require __DIR__ . '/api/v1/guide.php';
    require __DIR__ . '/api/v1/invoices.php';
    require __DIR__ . '/api/v1/payments.php';
    require __DIR__ . '/api/v1/accounting.php';
    require __DIR__ . '/api/v1/pipeline.php';
    require __DIR__ . '/api/v1/estimates.php';
    require __DIR__ . '/api/v1/projects.php';
    require __DIR__ . '/api/v1/documents.php';
    require __DIR__ . '/api/v1/hr.php';
    require __DIR__ . '/api/v1/forecasting.php';
    require __DIR__ . '/api/v1/efris.php';
    require __DIR__ . '/api/v1/storefront.php';
    require __DIR__ . '/api/v1/billing.php';
    require __DIR__ . '/api/v1/referral-codes.php';
    require __DIR__ . '/api/v1/staff_transfers.php';
    require __DIR__ . '/api/v1/sales-reps.php';
    require __DIR__ . '/api/v1/referrals.php';
    require __DIR__ . '/api/v1/currency.php';
    require __DIR__ . '/api/v1/credits.php';
    require __DIR__ . '/api/v1/public.php';
});
