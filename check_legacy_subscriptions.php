...
foreach (Plan::withTrashed()->orderBy('sort_order')->get() as $p) {
    echo "#{$p->id} slug={$p->slug} name={$p->name} type={$p->type} active=".($p->is_active?1:0)." sort={$p->sort_order} price=".$p->price_monthly_usd.PHP_EOL;
}

echo PHP_EOL."=== ALL SUBSCRIPTIONS ===".PHP_EOL;
$subs = Subscription::with('plan')->get();
foreach ($subs as $s) {
    echo "sub#{$s->id} biz#{$s->business_id} plan_id={$s->plan_id} (slug={$s->plan?->slug ?? 'NO_PLAN'}, name={$s->plan?->name ?? 'NO_PLAN'}) status=".$s->status->value." trial_used=".($s->trial_used?1:0)." fee_paid=".($s->onboarding_fee_paid?1:0).PHP_EOL;
}
...
echo "plans (all): ".Plan::withTrashed()->count().PHP_EOL;