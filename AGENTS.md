...
| 15 | **Deployment guardrails (READ BEFORE any deploy).** Before writing any deployment plan or touching staging/production, read `../DEPLOYMENT.md` in full. Every deploy requires a written deployment plan approved by Oscar first (todo list + rollback + verification). **Never** run `migrate:fresh`/`migrate:refresh`/`db:wipe`/drop database, `key:generate`, or deploy without a pre-wipe backup. Only `git add <exact paths>`, never `git add -A` on the shared repo. Migrations run with `--force` only. |
...
| Extract: services, traits, helpers, DTOs, FormRequests, API resources, jobs | “Temporarily” drop API behavior, migrations, or product work to green the gate |
